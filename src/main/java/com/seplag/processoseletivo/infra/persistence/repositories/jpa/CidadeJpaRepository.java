...
public interface CidadeJpaRepository extends JpaRepository<CidadeEntity, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT EXISTS(SELECT 1 FROM cidade WHERE cid_nome = :nome AND cid_uf = :uf)")
    boolean existsByCidNomeAndCidUf(String nome, String uf);
}
